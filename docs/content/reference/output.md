...
describes what users get. Pick a format with `-o`, or let sinanews choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
sinanews <command> -o table   # aligned columns for reading
sinanews <command> -o jsonl   # one JSON object per line, for piping
sinanews <command> -o json    # a single JSON array
sinanews <command> -o csv     # spreadsheet friendly
sinanews <command> -o tsv     # tab-separated
sinanews <command> -o url     # just the URL column
sinanews <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
sinanews <command> --fields id,title,url
```
...
```bash
sinanews <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
sinanews <command>            # a table, because this is a terminal
sinanews <command> | wc -l    # JSONL, because this is a pipe
```
